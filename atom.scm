...
(module atom
(atom-doc-encoding atom-doc-root
 atom-ns-prefixes
 author? author-email author-name author-uri
 category? category-contents category-label category-scheme category-term
 content? content-contents content-kind content-source
 content-text content-type content-xhtml content-xml
 contributor? contributor-email contributor-name contributor-uri
 entry? entry-author entry-authors entry-categories entry-content entry-contributors
 entry-id entry-links entry-published entry-rights entry-source
 entry-summary entry-title entry-updated entry=?
 feed? feed-author feed-authors feed-categories feed-contributors feed-entries
 feed-generator feed-icon feed-id feed-links feed-logo feed-rights feed-subtitle
 feed-title feed-updated
 generator? generator-agent generator-uri generator-version
 icon? icon-uri
 id=?
 link? link-contents link-length link-relation link-title link-type link-uri link-uri-language
 logo? logo-uri
 make-atom-doc
 make-author make-category make-content make-contributor make-entry
 make-feed make-generator make-icon make-link make-logo make-rights
 make-source make-subtitle make-summary make-title
 read-atom-doc read-atom-entry read-atom-feed
 rights? rights-text rights-type rights-xhtml
 source? source-author source-authors source-categories source-contributors
 source-generator source-icon source-id source-links source-logo source-rights
 source-subtitle source-title source-updated
 subtitle? subtitle-text subtitle-type subtitle-xhtml
 summary? summary-text summary-type summary-xhtml
 title? title-text title-type title-xhtml
 write-atom-doc
 )
...
(define (read-atom-doc p)
  (define (drop-preamble doc)
    (drop-while (lambda (x)       ;; or (find-tail sxml:element? (cdr doc))
                ...
                (cdr doc)))
  (let* ((doc (ssax:xml->sxml p (atom-ns-prefixes)))
...
(define content-kind
  (let ((rx:xml (irregex "[+/]xml$" 'i))
        (rx:text (irregex "^text/" 'i)))
    (lambda (C)
        ...
        (if src
            ...
            (let ((type (content-type C)))
              (cond ((string=? type "text/plain") 'text)
                    ...
                    ((irregex-search rx:xml type) 'xml) ; text/xml is XML
                    ((irregex-search rx:text type) 'textual)
                    (else 'binary))))))))
...
(define (atom-error where msg . args)
  (signal (make-composite-condition
           (make-property-condition 'atom)
           (make-property-condition 'exn 'message msg 'arguments args 'location where))))
